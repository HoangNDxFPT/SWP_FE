              ...
              {currentQuestion && (
                  ...
                  return (
                    <div
                      ...
                      key={question.id}
                      className={`${question.isInjectionQuestion ? 'bg-orange-50 border-orange-200' : 'bg-white'} p-5 rounded-lg shadow-sm border ${
                        isUnanswered ? 'border-2 border-yellow-400' : question.isInjectionQuestion ? 'border-orange-200' : 'border-gray-200'
                      } transition-all duration-200`}
                    >
                      <div className="font-semibold mb-3 flex items-start">
                        <span className={`inline-flex justify-center items-center w-7 h-7 ${
                          question.isInjectionQuestion 
                            ? 'bg-orange-100 text-orange-800' 
                            : isUnanswered ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'
                        } rounded-full mr-3 font-medium text-sm shrink-0`}>
                          {question.isInjectionQuestion ? '💉' : index + 2}
                        </span>
                        <div>
                          <span className="text-gray-800">{question.questionText}</span>
                          <div className={`text-sm mt-1 font-medium flex items-center gap-2 ${
                            question.isInjectionQuestion ? 'text-orange-600' : 'text-blue-600'
                          }`}>
                            ...
                            <span>
                              {question.isInjectionQuestion 
                                ...
                                : `Về chất: ${question.substanceName}`
                              }
                            </span>
                            {!question.isInjectionQuestion && question.substanceDescription && (
