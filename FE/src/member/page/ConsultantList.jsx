...
function ConsultantList() {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/Users')
            .then(res => res.json())
            .then(data => {
                const filtered = data.filter(u => u.role_id === 4 || u.role.toLowerCase === 'consultant');
    ...
    const isAvailableSchedule = (schedules, consultantId, date, time) =>
        !schedules.find(
            s => String(s.consultant_id) === String(consultantId)
                && s.work_date === date
                && s.start_time === time
                && !s.is_available
        );
    ...
    const filteredConsultants = useMemo(() => {
        return consultants.filter(c => {
            ...
            return matchName && isAvailableSchedule(schedules, c.id, date, time);
        });
    }, [consultants, search, date, time, schedules]);
